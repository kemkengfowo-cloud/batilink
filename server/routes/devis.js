...
router.put('/:id/terminer', auth, async (req, res) => {
  try {
    ...
    res.json({
      devis,
      message: `Travaux valides ! L artisan recevra ${new Intl.NumberFormat('fr-FR').format(devis.montantArtisan)} FCFA (90%). Batilink percoit ${new Intl.NumberFormat('fr-FR').format(devis.montantCommission)} FCFA (10%).`
    });
  } catch(err) { res.status(500).json({ message: err.message }); }
});
