...
router.post('/demandes', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: 'kemkengpowo@byh-cm.com', subject: 'BYH - Nouvelle demande conducteur', html: '<p>Nouvelle demande: ' + titreChantier + '</p>' }).catch(function(e) { console.error(e.message); });
    res.status(201).json({ message: 'Demande envoyee ! BYH va vous contacter sous 24h.', demande });
...
router.get('/mes-offres', auth, async (req, res) => {
  try {
    ...
    const demandes = await DemandeConducteur.find({ 'offres.conducteur': req.user.id }).populate('client','name city').sort({ createdAt: -1 });
    const offres = demandes.map(function(d) { return { demandeId: d._id, titreChantier: d.titreChantier, description: d.description, localisation: d.localisation, ville: d.ville, typeChantier: d.typeChantier, dateDebut: d.dateDebut, dateFin: d.dateFin, superficie: d.superficie, client: d.client, offre: d.offres.find(function(o) { return o.conducteur.toString() === req.user.id; }) }; }).filter(function(o) { return o.offre && o.offre.statut === 'envoyee'; });
    res.json(offres);
  } catch(err) { res.status(500).json({ message: err.message }); }
});
...
router.get('/mes-chantiers', auth, async (req, res) => {
  try {
    const demandes = await DemandeConducteur.find({ $or: [{ conducteur: req.user.id }, { conducteurRetenu: req.user.id }], statut: { $in: ['en_cours','conducteur_valide','valide_client','contrat_client','contrat_conducteur','conducteur_accepte'] } }).populate('client','name phone email city').sort({ createdAt: -1 });
    res.json(demandes);
  } catch(err) { res.status(500).json({ message: err.message }); }
});
...
router.put('/offres/:demandeId/repondre', auth, async (req, res) => {
  try {
    ...
    const offre = demande.offres.find(function(o) { return o.conducteur.toString() === req.user.id; });
    ...
    if (reponse === 'acceptee') {
      ...
      demande.offres.forEach(function(o) { if (o.conducteur.toString() !== req.user.id && o.statut === 'envoyee') { o.statut = 'expiree'; } });
      const conducteur = await User.findById(req.user.id);
      notifyAdmins('conducteur_accepte_offre', { demandeId: demande._id, titreChantier: demande.titreChantier });
      sendEmail({ to: 'kemkengpowo@byh-cm.com', subject: 'OK - Conducteur accepte', html: '<p>Conducteur accepte: ' + demande.titreChantier + '</p>' }).catch(function(e) { console.error(e.message); });
    }
    await demande.save();
    res.json({ message: reponse === 'acceptee' ? 'Offre acceptee !' : 'Offre refusee.', demande });
  } catch(err) { res.status(500).json({ message: err.message }); }
});
  ...
  try {
    ...
    for (let i = 0; i < conducteurs.length; i++) {
      ...
      if (dejaOffert === undefined) {
        ...
        if (cond) sendEmail({ to: cond.email, subject: 'BYH - Nouvelle offre mission', html: '<p>Bonjour ' + cond.name + ', mission proposee: ' + demande.titreChantier + ' a ' + demande.ville + '. Tarif: ' + c.tarifjour + ' FCFA/jour.</p>' }).catch(function(e) { console.error(e.message); });
      }
    }
...
router.put('/admin/demandes/:id/envoyer-contrat-conducteur', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: demande.conducteurRetenu.email, subject: 'CONTRAT - A signer', html: '<p>Bonjour ' + demande.conducteurRetenu.name + ', votre contrat est pret. Connectez-vous.</p>' }).catch(function(e) { console.error(e.message); });
...
router.put('/admin/demandes/:id/proposer-client', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: demande.client.email, subject: 'OK - Conducteur trouve', html: '<p>Bonjour ' + demande.client.name + ', conducteur trouve pour ' + demande.titreChantier + ': ' + demande.conducteurRetenu.name + '.</p>' }).catch(function(e) { console.error(e.message); });
...
router.put('/admin/demandes/:id/envoyer-contrat-client', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: demande.client.email, subject: 'CONTRAT - A signer client', html: '<p>Bonjour ' + demande.client.name + ', votre contrat pour ' + demande.titreChantier + ' est pret.</p>' }).catch(function(e) { console.error(e.message); });
...
router.put('/admin/demandes/:id/activer', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: demande.conducteurRetenu.email, subject: 'START - Mission demarree', html: '<p>Bonjour ' + demande.conducteurRetenu.name + ', votre mission ' + demande.titreChantier + ' est active !</p>' }).catch(function(e) { console.error(e.message); });
...
router.post('/chantiers/:id/rapports', auth, async (req, res) => {
  try {
    ...
    sendEmail({ to: demande.client.email, subject: 'RAPPORT ' + typeLabel + ' - ' + demande.titreChantier, html: '<p>Rapport soumis. Avancement: ' + avancement + '%.</p>' }).catch(function(e) { console.error(e.message); });
