...
export default function CreateProject() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Essayer sans photos si multipart échoue
      try {
        const res = await api.post('/projects', form);
        setArtisansNotifies(res.data.artisansNotifies || 0);
        setTimeout(() => navigate('/mes-projets'), 3000);
      } catch(err2) {
        setError(err2.response?.data?.message || 'Erreur lors de la publication');
      }
    }
    setLoading(false);
  };
...
cat > client/src/pages/CreateProject.jsx << 'EOF'
import React, { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import api from '../utils/api';
import { CATEGORIES, VILLES } from '../utils/helpers';
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl border border-gray-100 p-10 max-w-md w-full text-center shadow-xl">
          ...
          <p className="text-gray-400 text-sm mb-6">Redirection automatique dans 3 secondes...</p>
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <button onClick={() => setBesoinEvaluation(false)}
              ...
              <p className="text-gray-500 text-sm leading-relaxed">Je connais les travaux a realiser. Je publie mon projet et recois des devis.</p>
            ...
            <button onClick={() => navigate('/visites/demander')}
              ...
              <p className="text-gray-500 text-sm leading-relaxed">Je ne sais pas exactement quels travaux sont necessaires. Un technicien evalue sur place.</p>
    ...
    <div className="min-h-screen bg-gray-50">
      <div style={{background:'linear-gradient(135deg, #0a1628 0%, #0d2044 100%)'}} className="py-10">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <p className="text-blue-200">Les artisans disponibles recevront une notification automatique</p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <textarea required value={form.description} onChange={e=>set('description',e.target.value)} rows={5}
                ...
                placeholder="Decrivez precisement les travaux, materiaux souhaites, contraintes..."/>
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-blue-300 transition-colors">
                <input type="file" multiple accept="image/*" onChange={handlePhotos}
                  className="hidden" id="photos-input"/>
                <label htmlFor="photos-input" className="cursor-pointer">
                  ...
                  <p className="text-gray-400 text-xs mt-1">JPG, PNG — max 5 photos — 5MB chacune</p>
                </label>
                {photos.length > 0 && (
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
              ✅ Apres publication, tous les artisans disponibles dans votre ville seront notifies automatiquement.
            </div>
