...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (user.role === 'client') {
      api.get('/projects/my')
        .then(res => setData({ projects: res.data }))
        .finally(() => setLoading(false));
    } else if (user.role === 'artisan') {
      Promise.all([api.get('/artisans/me'), api.get('/projects?limit=6'), api.get('/missions?limit=4')])
        .then(([a, p, m]) => setData({ artisan: a.data, projects: p.data.projects, missions: m.data.missions }))
        .catch(() => setData({ artisan: null, projects: [], missions: [] }))
        .finally(() => setLoading(false));
    } else if (user.role === 'entreprise') {
      Promise.all([api.get('/entreprises/me'), api.get('/missions/my')])
        .then(([e, m]) => setData({ entreprise: e.data, missions: m.data }))
        .catch(() => setData({ entreprise: null, missions: [] }))
        .finally(() => setLoading(false));
    }
  }, [user]);
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                  ...
                  user.role==='artisan'?'bg-green-100 text-green-700':
                  'bg-purple-100 text-purple-700'}`}>
                  {user.role==='client'?'🏠 Client':user.role==='artisan'?'🔨 Technicien':'🏢 Entreprise BTP'}
                </span>
              </div>
              <p className="text-gray-500 mt-1">{user.city} • {user.email}</p>
            </div>
          </div>
        </div>
      </div>
...
function ClientDashboard({ projects }) {
  const stats = [
    { label:'Projets publiés', value:projects.length, icon:'📋', color:'bg-blue-50 text-blue-700' },
    { label:'Projets ouverts', value:projects.filter(p=>p.statut==='ouvert').length, icon:'🟢', color:'bg-green-50 text-green-700' },
    { label:'En cours', value:projects.filter(p=>p.statut==='en_cours').length, icon:'⚡', color:'bg-yellow-50 text-yellow-700' },
    { label:'Budget total', value:formatBudget(projects.reduce((s,p)=>s+p.budget,0)), icon:'💰', color:'bg-purple-50 text-purple-700' },
  ];
  return (
    <div className="space-y-8">
      ...
      {projects.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
          ...
          <p className="text-gray-400 mb-8">Publiez votre premier projet pour recevoir des offres d'artisans</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link to="/artisans" className="bg-white rounded-2xl border border-gray-100 p-6 hover:border-blue-200 hover:shadow-md transition-all group">
          <div className="flex items-center gap-4">
            ...
            <svg className="w-5 h-5 text-gray-300 ml-auto group-hover:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
          </div>
        </Link>
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-display font-bold text-gray-900">Missions de location disponibles</h2>
            ...
            {missions.map(m=>(
              <div key={m._id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:border-blue-200 hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-3">
                  <span className="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold">{m.typePersonnel?.join(', ')}</span>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-display font-bold text-gray-900">Projets clients disponibles</h2>
...
function EntrepriseDashboard({ entreprise, missions }) {
  const stats = [
    { label:'Missions publiées', value:missions.length, icon:'📋', color:'bg-blue-50 text-blue-700' },
    { label:'Missions ouvertes', value:missions.filter(m=>m.statut==='ouverte').length, icon:'🟢', color:'bg-green-50 text-green-700' },
    { label:'En cours', value:missions.filter(m=>m.statut==='en_cours').length, icon:'⚡', color:'bg-yellow-50 text-yellow-700' },
    { label:'Budget total', value:formatBudget(missions.reduce((s,m)=>s+m.remuneration,0)), icon:'💰', color:'bg-purple-50 text-purple-700' },
  ];
  return (
    <div className="space-y-8">
      {/* Profil entreprise */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link to="/create-mission" className="bg-blue-600 rounded-2xl p-6 text-white hover:bg-blue-700 transition-colors group">
          ...
          <div className="mt-4 flex items-center gap-2 text-blue-200 font-semibold text-sm group-hover:gap-3 transition-all">
            <span>Publier une mission</span><span>→</span>
          </div>
        </Link>
        <Link to="/artisans" className="bg-white rounded-2xl border-2 border-blue-200 p-6 hover:bg-blue-50 transition-colors group">
          ...
          <p className="text-gray-500 text-sm">Parcourez notre réseau de techniciens disponibles</p>
          <div className="mt-4 flex items-center gap-2 text-blue-600 font-semibold text-sm group-hover:gap-3 transition-all">
            <span>Voir les techniciens</span><span>→</span>
          </div>
        </Link>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-display font-bold text-gray-900">Mes missions de location</h2>
          <Link to="/create-mission" className="px-5 py-2.5 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-colors text-sm shadow-lg shadow-blue-600/20">+ Nouvelle mission</Link>
        </div>
        {missions.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
            <div className="text-6xl mb-4">👷</div>
            <h3 className="text-xl font-display font-bold text-gray-700 mb-2">Aucune mission publiée</h3>
            <p className="text-gray-400 mb-8">Publiez une mission pour trouver du personnel qualifié</p>
            ...
            {missions.map(m=>(
              <div key={m._id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:border-blue-200 hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${m.statut==='ouverte'?'bg-green-50 text-green-700':'bg-gray-100 text-gray-500'}`}>
