...
export default function DemandePersonnelDetail() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put('/demandes-personnel/' + id, {
        ...form,
  ...
  const handleAccepterPrix = async (montant) => {
    try {
      await api.put('/demandes-personnel/' + id + '/accepter-accord', { montant: montant });
  ...
  const handleContreOffre = async () => {
    ...
    try {
      await api.put('/demandes-personnel/' + id + '/contre-offre', {
        montant: parseInt(montantContreOffre),
  ...
  const propositions = demande.propositions || [];
  const derniereProposition = propositions.length > 0 ? propositions[propositions.length - 1] : null;
  const dernierePropositionAdmin = derniereProposition && derniereProposition.role === 'admin' ? derniereProposition : null;
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-2xl font-display font-black text-white">
                {demande.nombrePersonnes} {demande.typePersonnel && demande.typePersonnel.join(', ')} — {demande.ville}
              </h1>
              <p className="text-blue-300 mt-1">{formatDate(demande.dateDebut)} → {formatDate(demande.dateFin)}</p>
            </div>
            <span className={'px-4 py-2 rounded-xl text-sm font-bold ' + (statut ? statut.color : '')}>
              {statut && statut.icon} {statut && statut.label}
            </span>
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 rounded-xl">
                ...
                <div className="flex flex-wrap gap-1">
                  {demande.typePersonnel && demande.typePersonnel.map(t => (
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {TYPES_PERSONNEL.map(t => (
                    <button key={t} type="button" onClick={() => toggleType(t)}
                      className={'px-3 py-2 rounded-xl text-sm font-semibold border-2 transition-all ' + (form.typePersonnel && form.typePersonnel.includes(t) ? 'border-blue-500 bg-blue-500 text-white' : 'border-gray-200 text-gray-600')}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" min="1" value={form.nombrePersonnes} onChange={e => setForm(f => ({...f, nombrePersonnes: e.target.value}))} className={inputCls}/>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Date debut</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Budget (FCFA)</label>
                <input type="number" value={form.budgetPropose} onChange={e => setForm(f => ({...f, budgetPropose: e.target.value}))} className={inputCls}/>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                <textarea value={form.description} rows={3} onChange={e => setForm(f => ({...f, description: e.target.value}))} className={inputCls + ' resize-none'}/>
              </div>
              <div className="flex gap-3">
                <button onClick={handleSave} disabled={saving} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 disabled:opacity-50">
                  {saving ? 'Enregistrement...' : 'Enregistrer'}
                </button>
                <button onClick={() => setEditing(false)} className="px-6 py-3 border-2 border-gray-200 text-gray-600 rounded-xl font-semibold">
                  Annuler
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              {propositions.map((p, i) => (
                <div key={i} className={'p-4 rounded-xl ' + (p.role === 'admin' ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50')}>
          ...
          <div className="bg-white rounded-2xl border-2 border-blue-200 p-6">
            ...
            {!showContreOffre ? (
              <div className="flex gap-3 flex-wrap">
                <button onClick={() => handleAccepterPrix(dernierePropositionAdmin.montant)}
                ...
                <button onClick={() => setShowContreOffre(true)}
                  className="flex-1 py-3 bg-amber-50 text-amber-700 border-2 border-amber-200 rounded-xl font-bold">
                  Faire une contre-offre
                </button>
              </div>
            ) : (
              <div className="space-y-3">
                <input type="number" value={montantContreOffre}
                  onChange={e => setMontantContreOffre(e.target.value)}
                  className={inputCls} placeholder="Votre montant (FCFA)"/>
                <input type="text" value={messageContreOffre}
                  onChange={e => setMessageContreOffre(e.target.value)}
                  className={inputCls} placeholder="Message (optionnel)"/>
                <div className="flex gap-3">
                  <button onClick={handleContreOffre}
                  ...
                  <button onClick={() => setShowContreOffre(false)}
                    className="px-5 py-3 border-2 border-gray-200 rounded-xl font-semibold text-gray-600">
                    Annuler
                  </button>
                </div>
              </div>
